...
use crate::domain::models::{
    Connection, DeletedItem, Group, Project, ProjectCommand, ProjectData, current_unix_ts,
};
...
fn ssh_trash_snapshot(connection_id: &str) -> (String, DeletedItem) {
    let project = Project::new("srv", "/opt/x", "").with_connection(connection_id);
    let id = project.id.clone();
    (id, DeletedItem::from_project(&project, "Work", 1))
}

fn connection_with_key(id: &str, key: &str) -> Connection {
    Connection {
        id: id.into(),
        name: id.into(),
        host: "h".into(),
        ssh_key_file: key.into(),
        ..Connection::default()
    }
}

#[test]
fn restore_with_id_conflict_keeps_connection_id() {
    let mut data = ProjectData::default();
    add_group_with_alias(&mut data, "Work", "").unwrap();
    data.connections
        .push(connection_with_key("cid-1", "keys/srv.key"));
    let (id, snapshot) = ssh_trash_snapshot("cid-1");
    ...
    assert_eq!(restored.connection_id, "cid-1");
...
#[test]
fn empty_trash_does_not_delete_connection_keys() {
    ...
    let (id_a, snapshot_a) = ssh_trash_snapshot("cid-a");
    let (_id_b, snapshot_b) = ssh_trash_snapshot("cid-b");
    std::fs::write(root.join("keys").join("a.key"), "enc-a").unwrap();
    std::fs::write(root.join("keys").join("b.key"), "enc-b").unwrap();
    let mut data = ProjectData::default();
    add_group_with_alias(&mut data, "Work", "").unwrap();
    data.connections
        .push(connection_with_key("cid-a", "keys/a.key"));
    data.connections
        .push(connection_with_key("cid-b", "keys/b.key"));
...
#[test]
fn soft_delete_keeps_connection_id_and_drop_key_file_dedups() {
    let mut data = trash_data();
    data.connections
        .push(connection_with_key("cid-1", "keys/srv.key"));
    data.groups[0]
        .projects
        .push(Project::new("srv", "/opt/x", "").with_connection("cid-1"));
    let removed = remove_project(&mut data, "srv", None, false).unwrap();
    assert_eq!(data.trash.last().unwrap().connection_id, "cid-1");
    assert_eq!(removed.connection_id, "cid-1");
    assert!(data.pending_key_deletes.is_empty());
    drop_key_file(&mut data, "keys/missing.key");
